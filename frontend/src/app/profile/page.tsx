...
function ProfileContent() {
    ...
    const [formData, setFormData] = useState<ProfileFormData>({
        name: '', height: '', gender: '', mbti: '',
        occupation: '', nickname: '', city: ''
    });
    ...
    const handleSave = async (e: FormEvent) => {
        ...
        } else if (isFirstCompletion && profileImagePreviews.length === 0) {
            // 첫 프로필 작성 시에만 이미지 필수
            setIsSaving(false);
    ...
    const handleCancel = () => {
        ...
        if (userProfile) {
            setFormData({
                name: userProfile.name || '', height: userProfile.height?.toString() || '',
                gender: userProfile.gender || '', mbti: userProfile.mbti || '',
                occupation: userProfile.occupation || '', nickname: userProfile.nickname || '', city: userProfile.city || ''
            });
        }
        setError(null);
    };
    ...
    const handleBuyCredit = async () => {
        ...
        try {
            // 서버에 credit 추가 요청을 보냄
            const response = await axiosInstance.post('/api/profile/buy-credit', { amount: 10 });
            
            if (response.status === 200) {
                // 성공적으로 credit이 추가되면 프로필 업데이트
                const updatedProfile = { ...userProfile, credit: (userProfile.credit || 0) + 10 };
                setUserProfile(updatedProfile);
                alert('10 크레딧이 성공적으로 추가되었습니다.');
            }
        } catch (err: any) {
            console.error('크레딧 구매 중 오류 발생:', err);
            ...
            <div className={cardStyle}>
                <div className="flex justify-between items-center mb-6">
                    ...
                    {!isEditing && (
                        <div className="flex items-center space-x-4">
                            {/* Credit 표시 */}
                            <div className="bg-amber-600 px-3 py-1 rounded-full text-sm font-medium">
                                <span>크레딧: {userProfile?.credit || 0}</span>
                            </div>
                            <button 
                                onClick={handleLogout}
                                className={`${buttonBaseStyle} bg-red-600 hover:bg-red-700 text-white text-sm`}
                            >
                                Logout
                            </button>
                        </div>
                ...
                {isEditing ? (
                    <form onSubmit={handleSave} className="space-y-4">
                        ...
                        <div className="mt-8">
                            ...
                            <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 bg-gray-800">
                                <p className="text-sm text-amber-400 mb-2">프로필 사진 추가하기 ({3-profileImagePreviews.length}장 더 추가 가능):</p>
                                <input
                                    type="file"
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (e.target.files && e.target.files.length > 0) {
                                            ...
                                            // 파일 이름 UUID로 변경
                                            const renamedFiles = files.map(file => renameFileWithUUID(file));
                                            
                                            // 업로드 대기 이미지와 미리보기 업데이트
                                            setUploadedProfileImages(prev => [...prev, ...renamedFiles]);
                                            
                                            // 미리보기 URL 생성
                                            const urls = files.map(file => URL.createObjectURL(file));
                                            setProfileImagePreviews(prev => [...prev, ...urls]);
                                            
                                            console.log(`🚨 파일 ${files.length}개가 추가됨`);
                                        }
                    ...
                    <div className="space-y-6">
                        {/* 중요 정보 세션 - 불필요한 정보 제거하고 재구성 */}
                        <div className="p-4 bg-gray-900 rounded-xl">
                            <h2 className="text-xl font-semibold text-amber-400 mb-3">기본 정보</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-gray-800 p-3 rounded-lg">
                                    <span className={`${labelStyle} text-amber-300`}>닉네임</span>
                                    <p className={`${valueStyle} text-xl`}>{userProfile?.nickname ?? '-'}</p>
                                </div>
                                <div className="bg-gray-800 p-3 rounded-lg">
                                ...
                                <div className="bg-gray-800 p-3 rounded-lg">
                                    <span className={`${labelStyle} text-amber-300`}>나이</span>
                                    <p className={`${valueStyle} text-xl`}>{userProfile?.age ?? '-'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-900 rounded-xl">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div><span className={labelStyle}>MBTI</span><p className={valueStyle}>{userProfile?.mbti?.toUpperCase() ?? '-'}</p></div>
                                <div><span className={labelStyle}>가입일</span><p className={valueStyle}>{userProfile?.createdAt ? new Date(userProfile.createdAt).toLocaleDateString() : '-'}</p></div>
                            </div>
                        </div>
                        
                        {/* Credit 구매 섹션 추가 */}
                        <div className="p-4 bg-gray-900 rounded-xl">
                            <h2 className="text-xl font-semibold text-amber-400 mb-3">크레딧</h2>
                            <div className="flex justify-between items-center">
                                <div>
                                    <span className={labelStyle}>현재 크레딧</span>
                                    <p className={`${valueStyle} text-xl`}>{userProfile?.credit || 0}</p>
                                </div>
                                <button
                                    onClick={handleBuyCredit}
                                    className="bg-amber-500 hover:bg-amber-600 text-black px-4 py-2 rounded-lg font-medium"
                                >
                                    크레딧 구매 (+10)
                                </button>
                            </div>
                        ...
                        <div className="border-t border-slate-700 pt-6 flex flex-col sm:flex-row justify-end items-center space-y-4 sm:space-y-0">
                            <button 
